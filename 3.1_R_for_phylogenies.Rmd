...
The plot above is a generic X-Y plotting function for plotting R objects such as x and y coordinates. The plot below is a phylogeny plotting function which plots phylogenetic trees such as using the edge lengths of the phylogeny tree to draw the branches, drawing nodes, etc.
...
tr_primates
```
This gives the labels of the tips of the phylogenetic tree but does not include branch lengths.
```{r}
str("tr_primates")
```
This gives the primary species (possibly family) we are going to be looking at in the phylogenetic tree.
# No, this just tells us that `tr_primates` is a character or string variable, rather than numeric (i.e. words rather than numbers).
```{r}
str(tr_primates)
```
This gives the lengths of the branches, (I belive) the number of nodes, the different tips on the phylogenetic tree, the different classes present within the chosen family as well as the order of organization based on clades.
```{r}
attributes(tr_primates)
```
This further organizes the previous information by presenting the attributes of the phylogenetic tree in terms of names of characteristics.
```{r}
names(tr_primates) 
```
This gives the characteristics that are going to be on the phylogenetic tree such as the branch length, nodes, and the labels on the tips.
...
```{r}
newick_str = "(((Angiosperms, Gymnosperms), Ferns), Lycopodiales);"
#Here, we are defining a character variable. 
...
I would assume that the botanically correct thing would be for Cabbage and Kale to be more related.
...
Yes, it is easier to see relationships in the plotted tree format.
...
Possibly not because it depends on which species is "older" vs "younger." If Wheat is the "younger" species then this tree would be accurate for these four taxa but if not, this would be incorrect. This is what I can tell from the Newick but it would be easier to tell from the tree.
...
```{r}
newick_str = "((((Cabbage), Kale), Corn), Wheat);"
tr_plants = read.tree(text=newick_str)
plot(tr_plants)
```
...
```{r}
newick_str = "((((((Cabbage), Kale), Corn), Wheat), Radish), Sequoia);"
tr_plantserr = read.tree(text=newick_str)
plot(tr_plantserr)
```

```{r}
attributes(tr_plantserr)
```
...
str(tr_plantserr)
```
# Make sure to call the correct variable. You haven't created `tr_plantscorr` yet, so this code won't work.

```{r}
newick_str = "((((((Radish), Cabbage), Kale), Corn), Wheat), Sequoia);"
tr_plantscorr = read.tree(text=newick_str)
plot(tr_plantscorr)
```

```{r}
attributes(tr_plantscorr)
```
...
In the erraneous tree, the tips are not arranged based on how "old" or "young" each species is which makes it incorrect.
...
According to the "str" and "attr" commands, the erraneous and correct tree do not differ in tips, Nnodes, or edges.

## Branching order and branching lengths

Returning to the original tree, What was added to the tree and the plot of the tree?
It seems as though nothing was added here when looking at only the phylogentic tree but when analyzing the "str" command, it can be seen that node labels and edge lengths were added.
```{r}
...
Node labels and edge lengths were added to the tree and the plot of the tree
```{r}
...
```{r}
newick_str = "((Humans:12.5, Chimps:12.5, Gorillas:14.0):1.0, Pongo_abelii:14.5, Pongo_tapanuliensis:14.5):1.0;"
tr_primate = read.tree(text=newick_str)
plot(tr_primate)
```
...
```{r}
plot(tr_plantscorr, type="cladogram")
plot(tr_plantscorr, type="fan")
plot(tr_plantscorr, type="unrooted")
plot(tr_plantscorr, type="radial")
```
...
```{r}
layout(matrix(1:4, 2, 2))
plot(tr_plantscorr, type="unrooted", edge.width=5)
plot(tr_plantscorr, type="unrooted", edge.width=5, edge.color="blue")
plot(tr_plantscorr, type="unrooted", edge.width=5, edge.color="blue", lab4ut="horizontal")
plot(tr_plantscorr, type="unrooted", edge.width=5, edge.color="blue", lab4ut="axial")
```

```{r}
# rotate clades containing nodes 3 and 4:
trnew_plants <- rotate(tr_plantscorr, c("Cabbage", "Kale"))
plot(trnew_plants)
# compare the results:
par(mfrow=c(1,2)) # split graphical device
plot(tr_plantscorr) # plot old tre
plot(trnew_plants) # plot new tree
```

```{r}
layout(matrix(1:4, 2, 2))
plot(tr_plantscorr, main = "normal")
plot(ladderize(tr_plantscorr), main = "right-ladderized")
plot(ladderize(tr_plantscorr, FALSE), main = "left-ladderized")
layout(matrix(1, 1))
```
...
```{r}
newick_fn = "tr_plantscorr"
write.tree(tr_plantscorr, file=newick_fn)
```
...
plot(tr_lectureFULL, type="phylogram", direction=c("upwards"))
```
...
plot(tr_lectureSUB1, type="phylogram", direction=c("upwards"))
newick_lecture2 = "(((I,J),K),(L,M));"
tr_lectureSUB2 = read.tree(text=newick_lecture2)
plot(tr_lectureSUB2, type="phylogram", direction=c("upwards"))
```
...
What differs most about the trees is that some of the trees are more spaced out which makes it easier to see the relationship between different species, but for the trees with more species it is hard to see some of the relationships because the trees are so clustered. This relates to the stored tree because in that, the trees with more species also were more clustered which made it difficult to understand the relationship between some of them. This could also just mean that the code needs some altering in order to space out some of the branches.
...
Some of the taxa were excluded because for some of them, there is some data about them such as bangia and for others such as aspicllum there is not even one Google search that arises. Due to the lack of data present on such taxa it was difficult to include them. This shows that although the history of botany has been around for a long time, there is still more work to be done and more research to be performed in collecting the data of such "invisible" taxa. It seems as though many taxa relating to algae were excluded perhaps when they should not have been because I think that there is substantial data on such taxa (i.e., fucus and bangia).
...
W
tree_phylomatic_W <- phylomatic(taxa = p211, get = 'POST')
plot(tree_phylomatic_p211, no.margin = TRUE, type="phylogram")
newick_fn4 = "tree_phylomatic_W"
write.tree(tree_phylomatic_W, file=newick_fn4)
#repeating to get tree from Zanne2014
tree_phylomatic_W1 <- phylomatic(taxa = EG, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic_W1, no.margin = TRUE, type="phylogram")
newick_fn5 = "tree_phylomatic_W1"
write.tree(tree_phylomatic_W1, file=newick_fn5)
```
```{r}
W<-c("Illumina",
"Agrobacterium",
...
W
tree_phylomatic_W <- phylomatic(taxa = p211, get = 'POST')
plot(tree_phylomatic_W, no.margin = TRUE, type="cladogram")
newick_fn4 = "tree_phylomatic_W"
write.tree(tree_phylomatic_W, file=newick_fn4)
#repeating to get tree from Zanne2014
tree_phylomatic_W1 <- phylomatic(taxa = EG, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic_W1, no.margin = TRUE, type="cladogram")
newick_fn5 = "tree_phylomatic_W1"
write.tree(tree_phylomatic_W1, file=newick_fn5)
```
```{r}
newick_fn = "tree_phylomatic_W1"
write.tree(tree_phylomatic_W1, file=newick_fn5)
```

```{r}
pdf_fn = "tree_phylomatic_W1"
pdf(file=pdf_fn)
plot(tree_phylomatic_W1, type="unrooted", edge.width=5, edge.color="blue", lab4ut="axial")
dev.off()
```
```{r}
cmdstr = paste("open ", pdf_fn, sep="")
...
"Erigeron strigosus Muhl.exWilld.",
...
"Dryopteris intermedia (Muhl.exWilld.)Gray",
...
"Arabis laevigata (Muhl.exWilld.)Poir",
"Asclepias tuberosa L.",
"Athyrium filix-femina (L.)Roth")
A
tree_phylomatic_A <- phylomatic(taxa = A, get = 'POST')
plot(tree_phylomatic_A, no.margin = TRUE, type="cladogram")
newick_fn4 = "tree_phylomatic_A"
write.tree(tree_phylomatic_A, file=newick_fn4)
#repeating to get tree from Zanne2014
tree_phylomatic_A1 <- phylomatic(taxa = EG, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic_A1, no.margin = TRUE, type="cladogram")
newick_fn5 = "tree_phylomatic_A1"
write.tree(tree_phylomatic_A1, file=newick_fn5)
```
```{r}
A<-c("Acer rubrum L.",
"Carpinus caroliniana Walt.",
...
A
tree_phylomatic_A <- phylomatic(taxa = A, get = 'POST')
plot(tree_phylomatic_A, no.margin = TRUE, type="phylogram")
newick_fn4 = "tree_phylomatic_A"
write.tree(tree_phylomatic_A, file=newick_fn4)
#repeating to get tree from Zanne2014
tree_phylomatic_A1 <- phylomatic(taxa = EG, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic_A1, no.margin = TRUE, type="phylogram")
newick_fn5 = "tree_phylomatic_A1"
write.tree(tree_phylomatic_A1, file=newick_fn5)
```

```{r}
newick_fn = "tree_phylomatic_A1"
write.tree(tree_phylomatic_A1, file=newick_fn5)
```

```{r}
pdf_fn1 = "tree_phylomatic_A1"
pdf(file=pdf_fn1)
plot(tree_phylomatic_A1, type="unrooted", edge.width=5, edge.color="blue", lab4ut="axial")
dev.off()
```
```{r}
cmdstr = paste("open ", pdf_fn1, sep="")
system(cmdstr)
```

```

Everything worked great except for a few lines of code from 674 - 762. You're trying to call a variable called `p211`, but R is saying that it can't find a variable with that name. If you can fix this, push it back to GitHub and I can check it for full points.
